...
GITHUB_REPO = “cinepark-1974/AutoPost”  # 본인 저장소

def save_to_github_issue(keyword, content, category, github_token=None):
“””
생성된 글을 GitHub Issues에 저장
“””
try:
if not github_token:
return None
...
```
    # Issue 제목
    title = f"📝 {keyword} ({category}) - {datetime.now().strftime('%Y-%m-%d')}"
...
```
    ...
    st.warning(f"GitHub 저장 실패: {str(e)}")
    return None
```

def get_github_archives(github_token=None):
“””
GitHub Issues에서 저장된 글 목록 가져오기
“””
try:
if not github_token:
return []
...
try:
    if hasattr(st, 'secrets') and "GITHUB_TOKEN" in st.secrets:
        ...
        st.success("✅ GitHub Token 자동 로드")
    else:
        github_token = st.text_input(
            "GitHub Token (아카이브용)",
            type="password",
            help="영구 저장하려면 github.com에서 발급 (선택)"
        )
except:
    github_token = None
...
with col2:
    ...
    image_prompt = st.text_input(
        "이미지 키워드 (선택)",
        placeholder="예: sunset ocean beach, mountain landscape",
        help="비워두면 글 키워드 사용. 원하는 이미지를 구체적으로 영문으로 입력하세요"
    )
...
```
                ...
                image_info = None
                if include_image:
                    # 이미지용 키워드 결정 (별도 입력 있으면 우선 사용)
                    image_keyword = image_prompt.strip() if image_prompt.strip() else keyword
                    
                    if hf_token:
                        # Hugging Face Token 있으면 SD 시도
                        with st.spinner(f"AI가 이미지를 생성 중입니다... (키워드: {image_keyword})"):
                            try:
                                image_info = generate_sd_image(image_keyword, hf_token)
                                if image_info and 'image' in image_info:
                                    st.session_state.image_count += 1
                                else:
                                    # SD 실패 시 무료 이미지로 대체
                                    st.info("💡 AI 이미지 생성 실패. 무료 이미지로 대체합니다.")
                                    image_info = get_free_image(image_keyword)
                            except Exception as e:
                ...
                # GitHub Issues에 영구 저장
                if github_token:
                    with st.spinner("GitHub에 아카이브 중..."):
                        issue_url = save_to_github_issue(keyword, generated_content, category, github_token)
                        if issue_url:
                            st.success(f"✅ GitHub에 영구 저장됨: [링크 보기]({issue_url})")
                            post_data["github_url"] = issue_url
                        else:
                            st.warning("⚠️ GitHub 저장 실패. 세션에만 저장됩니다.")
                else:
                    st.info("💡 GitHub Token을 입력하면 영구 저장할 수 있습니다.")
...
```
# GitHub 아카이브 불러오기 버튼
col_archive1, col_archive2 = st.columns([3, 1])

with col_archive1:
    st.markdown("**세션 저장소** (임시 - 새로고침 시 사라짐)")

with col_archive2:
    if github_token and st.button("🔄 GitHub에서 불러오기"):
        with st.spinner("GitHub Issues에서 불러오는 중..."):
            archives = get_github_archives(github_token)
            if archives:
                st.success(f"✅ {len(archives)}개의 글을 찾았습니다!")
                st.session_state.github_archives = archives
            else:
                st.warning("저장된 글이 없거나 불러오기에 실패했습니다.")
...
if len(st.session_state.saved_posts) == 0:
    ...
    st.success(f"세션: {len(st.session_state.saved_posts)}개")
    ...
    for idx, post in enumerate(st.session_state.saved_posts):
        with st.expander(f"📄 {post['keyword']} ({post['category']}) - {post['timestamp']}"):
            ...
            with col_a:
                ...
                if 'github_url' in post:
                    st.markdown(f"**GitHub**: [영구 저장됨]({post['github_url']})")
    ...
    # 전체 삭제
    st.divider()
    if st.button("🗑️ 세션 전체 삭제", type="secondary"):
        st.session_state.saved_posts = []
        st.rerun()
...
if github_token:
    st.divider()
    st.markdown("### 📦 GitHub 아카이브 (영구 저장)")
    
    if 'github_archives' in st.session_state and st.session_state.github_archives:
        st.success(f"총 {len(st.session_state.github_archives)}개의 글이 GitHub에 저장되어 있습니다.")
        
        for issue in st.session_state.github_archives[:10]:  # 최근 10개만
            with st.expander(f"🔖 {issue['title']}"):
                st.markdown(f"**작성일**: {issue['created_at'][:10]}")
                st.markdown(f"**상태**: {issue['state']}")
                st.markdown(f"**링크**: [GitHub에서 보기]({issue['html_url']})")
                
                # 본문 미리보기 (첫 500자)
                preview = issue['body'][:500] + "..." if len(issue['body']) > 500 else issue['body']
                st.text_area("미리보기", preview, height=150, key=f"archive_{issue['number']}")
    else:
        st.info("🔄 'GitHub에서 불러오기' 버튼을 눌러 저장된 글을 확인하세요.")
```
